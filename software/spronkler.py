                ...
                if (newstart > start and newstart < end) or (newend > start or newend < end):
                    ...
                    while i < len(runtimes) and j < len(newruntimes):
                        ...
                        if (j_newstart >= i_start and j_newstart < i_end) or (j_newend > i_start and j_newend <= i_end):
                            self.log("Conflict: ({0} >= {2} and {0} < {3}) or ({1} > {2} and {1} <= {3})".format(j_newstart, j_newend, i_start, i_end))
            ...
            while True:
                try:
                        ...
                        if isinstance(conflict_result, self.MsgACK):
                            ...
                            while msg.schedule['nextrun'] < nowstamp:
                                ...
                                if msg.schedule['name'] == "Eight Thirty":
                                    self.log("{}, {}".format(addtime, datetime.datetime.fromtimestamp(msg.schedule['nextrun']).astimezone(pytz.timezone("America/Los_Angeles"))))
                    ...
                    while i < len(self.schedules):
                        start_time = datetime.datetime.strptime(self.schedules[i]['start_time'], self.dateformat).replace(year=2000).astimezone(pytz.utc)
                        end_time = datetime.datetime.strptime(self.schedules[i]['end_time'], self.dateformat).replace(year=2000).astimezone(pytz.utc)
